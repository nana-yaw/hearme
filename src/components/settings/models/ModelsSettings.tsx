...
import { ChevronDown, Gauge, Globe, RefreshCw, Search } from "lucide-react";
import type { ModelCardStatus } from "@/components/onboarding";
import { ModelCard } from "@/components/onboarding";
import { VoiceSetupWizard } from "./VoiceSetupWizard";
import { getGuidanceSearchText } from "@/lib/modelGuidance";
...
export const ModelsSettings: React.FC = () => {
  ...
  const filteredModels = useMemo(() => {
    ...
    return models.filter((model: ModelInfo) => {
      ...
      if (q) {
        const haystack =
          `${model.name} ${model.description} ${getGuidanceSearchText(model)}`.toLowerCase();
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {/* Voice setup: benchmark downloaded models on the user's own voice */}
                <button
                  type="button"
                  onClick={() => setVoiceSetupOpen(true)}
                  disabled={!models.some((m: ModelInfo) => m.is_downloaded)}
                  title={t("settings.models.voiceSetup.tooltip")}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg bg-logo-primary/20 text-logo-primary hover:bg-logo-primary/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Gauge className="w-3.5 h-3.5" />
                  <span>{t("settings.models.voiceSetup.button")}</span>
                </button>
