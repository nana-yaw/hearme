...
import { getModelGuidance } from "../../lib/modelGuidance";
        ...
        <div className="flex flex-col items-start flex-1 min-w-0">
          ...
          {guidance && (
            <div className="flex flex-col gap-0.5 mt-1">
              <p className="flex items-start gap-1.5 text-xs text-text/55">
                <ThumbsUp className="w-3 h-3 mt-0.5 shrink-0 text-logo-primary/70" />
                <span>
                  <span className="font-medium">
                    {t("modelSelector.guidance.bestFor")}
                  </span>{" "}
                  {guidance.bestFor.join(" · ")}
                </span>
              </p>
              {guidance.notFor && guidance.notFor.length > 0 && (
                <p className="flex items-start gap-1.5 text-xs text-text/55">
                  <ThumbsDown className="w-3 h-3 mt-0.5 shrink-0 text-text/40" />
                  <span>
                    <span className="font-medium">
                      {t("modelSelector.guidance.notFor")}
                    </span>{" "}
                    {guidance.notFor.join(" · ")}
                  </span>
                </p>
              )}
              {guidance.note && (
                <p className="text-xs text-text/45 italic">{guidance.note}</p>
              )}
            </div>
          )}
        </div>
