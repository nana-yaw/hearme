...
/// Mid-sentence capitalized words that recur across transcripts are probable
/// proper nouns the models keep guessing at — surfaced as one-click custom-word
/// suggestions. Sentence-initial capitals are skipped (every sentence starts
/// capitalized), as are pronoun contractions and anything already in the list.
pub fn proper_noun_candidates(
    transcripts: &[String],
    ...
    const PRONOUNS: &[&str] = &["i", "i'm", "i'll", "i've", "i'd"];
    let existing: Vec<String> = existing_words.iter().map(|w| w.to_lowercase()).collect();

    // key: lowercased word -> (display casing, count)
    ...
    for transcript in transcripts {
        ...
        for (i, raw) in words.iter().enumerate() {
            ...
            // Sentence-initial words are capitalized by grammar, not identity.
            ...
            if PRONOUNS.contains(&key.as_str()) || existing.contains(&key) {
                continue;
            }
            let entry = counts.entry(key).or_insert_with(|| (word.to_string(), 0));
            entry.1 += 1;
        }
    }
...
mod proper_noun_tests {
    use super::proper_noun_candidates;
    ...
    #[test]
    fn recurring_mid_sentence_names_are_suggested() {
        let transcripts = t(&[
            "send the invoice to Kwame tomorrow",
            "I told Kwame about the meeting in Kumasi",
            "the Kumasi office called again",
        ]);
        let out = proper_noun_candidates(&transcripts, &[], 10);
        // Equal counts tie-break alphabetically, so Kumasi sorts first.
        assert_eq!(out, vec!["Kumasi".to_string(), "Kwame".to_string()]);
    }

    #[test]
    fn sentence_initial_and_single_occurrences_are_skipped() {
        let transcripts = t(&["Tomorrow we ship. Tomorrow is fine", "we met Adjoa once"]);
        let out = proper_noun_candidates(&transcripts, &[], 10);
        assert!(out.is_empty(), "got {out:?}");
    }

    #[test]
    fn existing_words_and_pronouns_are_excluded() {
        let transcripts = t(&[
            "ask Kwame and I'll confirm with Kwame",
            "then I'll write it up",
        ]);
        let existing = t(&["kwame"]);
        let out = proper_noun_candidates(&transcripts, &existing, 10);
        assert!(out.is_empty(), "got {out:?}");
    }
}
