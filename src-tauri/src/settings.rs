...
pub struct AppProfile {
    /// macOS bundle identifier of the frontmost app this profile matches.
    pub bundle_id: String,
    /// Id of the post-processing prompt (from post_process_prompts) to use.
...
pub struct AppSettings {
    ...
    /// Fork: deterministic "period"->"." replacement with article guards.
    #[serde(default = "default_spoken_punctuation_enabled")]
    pub spoken_punctuation_enabled: bool,
    /// Fork: RNNoise pass over the finished recording before transcription.
    /// Experimental; the stored recording keeps the original audio.
    #[serde(default = "default_noise_suppression_enabled")]
    pub noise_suppression_enabled: bool,
    /// Experimental: type committed streaming text into the target app while
    /// the user is still speaking; the final cleaned text replaces it.
    #[serde(default)]
    pub live_typing_enabled: bool,
    /// Fork: UI accent color as "#rrggbb"; empty = the HearMe brand pink.
    #[serde(default)]
    pub accent_color: String,
    /// Fork: per-app post-processing prompt overrides, matched on the
    /// frontmost app's bundle id at transcription stop.
    #[serde(default)]
...
fn default_update_checks_enabled() -> bool {
    // Private fork: no phone-home by default, and an accepted auto-update would
    // replace this fork's binary with an upstream release. Can be enabled in UI.
    false
}
...
fn default_post_process_providers() -> Vec<PostProcessProvider> {
    // Private fork: cloud providers (OpenAI, Anthropic, Z.AI, OpenRouter, Groq,
    // Cerebras, AWS Bedrock) are removed so dictated text cannot leave this
    // machine. Only on-device Apple Intelligence and localhost (Ollama) remain.
...
fn default_model_for_provider(provider_id: &str) -> String {
    ...
    // The custom provider is pre-pointed at localhost Ollama; defaulting its
    // model to the docs/INSTALL.md recipe means `brew install ollama && ollama
    // pull qwen2.5:7b-instruct` is the only setup step. The load-path sync
    // fills this only where the stored value is empty — a user-chosen model is
    // never overwritten.
    if provider_id == "custom" {
        return "qwen2.5:7b-instruct".to_string();
    }
    String::new()
}
...
/// Bindings added by newer versions must appear in stores written by older
/// ones — additive only, existing customized combos are never touched.
...
fn ensure_post_process_defaults(settings: &mut AppSettings) -> bool {
    ...
    // Private fork: prune providers absent from the local-only default set. A
    // settings store written by upstream Handy still contains cloud providers
    // (and their API keys), and without pruning those stay selectable — dictated
    // text could silently leave the machine on an upgraded install.
    let allowed_ids: std::collections::HashSet<String> = default_post_process_providers()
        .into_iter()
        .map(|provider| provider.id)
        .collect();
    let providers_before = settings.post_process_providers.len();
    settings
        .post_process_providers
        .retain(|provider| allowed_ids.contains(&provider.id));
    changed |= settings.post_process_providers.len() != providers_before;

    let api_keys_before = settings.post_process_api_keys.len();
    settings
        .post_process_api_keys
        .retain(|id, _| allowed_ids.contains(id));
    changed |= settings.post_process_api_keys.len() != api_keys_before;

    let models_before = settings.post_process_models.len();
    settings
        .post_process_models
        .retain(|id, _| allowed_ids.contains(id));
    changed |= settings.post_process_models.len() != models_before;

    if !allowed_ids.contains(&settings.post_process_provider_id) {
...
pub fn get_default_settings() -> AppSettings {
    ...
    // ctrl+shift+space is the post-process default on Windows/Linux — the
    // command-mode combo must differ per OS or the second registration fails.
    #[cfg(target_os = "macos")]
    let default_command_mode_shortcut = "ctrl+shift+space";
    ...
    bindings.insert(
        ...
        ShortcutBinding {
            ...
            description: "Copy text, hold, speak an instruction (\"make this formal\") — the transformed text is pasted.".to_string(),
