...
#[tauri::command]
#[specta::specta]
pub fn change_live_typing_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.live_typing_enabled = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
pub fn get_frontmost_app_bundle_id() -> Option<String> {
    crate::app_context::frontmost_bundle_id()
}

#[tauri::command]
#[specta::specta]
pub fn change_app_profiles_setting(
    app: AppHandle,
    profiles: Vec<settings::AppProfile>,
) -> Result<(), String> {
    let mut current = settings::get_settings(&app);
    current.app_profiles = profiles;
...
pub fn change_accent_color_setting(app: AppHandle, color: String) -> Result<(), String> {
    // Empty resets to the brand default; otherwise strictly #rrggbb — the
    // value ends up in a CSS custom property, so never accept free text.
