...
impl TranscriptionManager {
    ...
    /// `allow_live_typing` is false for command-mode recordings: the spoken
    /// instruction is not dictation, and typing it would destroy the very
    /// selection command mode is about to transform.
    pub fn start_stream(&self, allow_live_typing: bool) {
        ...
        thread::spawn(move || manager.run_stream_worker(rx, worker_id, allow_live_typing));
    }
    ...
    fn run_stream_worker(
        ...
        // Live typing (experimental): types committed text while the user
        // speaks; erased again before the finalize reply so the action's
        // paste never interleaves with keystrokes. Spawned only after the
        // stream actually begins.
        let mut live_typer: Option<crate::live_typing::LiveTyper> = None;
        let stream_started = 'stream: {
            ...
            self.touch_activity();
            if allow_live_typing
                && settings.live_typing_enabled
                && crate::live_typing::allowed_for_paste_method(&settings.paste_method)
            {
                live_typer = Some(crate::live_typing::LiveTyper::spawn(&self.app_handle));
            }
            let mut typed_committed = String::new();
            let mut live_feeding = true;
            info!(
                        ...
                        match stream.feed(&pcm) {
                            Ok(update) => {
                                ...
                                if update.committed_changed || update.tentative_changed {
                                    ...
                                    if live_feeding {
                                        if let Some(typer) = &live_typer {
                                            use crate::live_typing::{
                                                live_typing_delta, LiveDelta,
                                            };
                                            match live_typing_delta(
                                                &typed_committed,
                                                &text.committed,
                                            ) {
                                                LiveDelta::Append(delta) => {
                                                    if delta.is_empty() {
                                                    } else if crate::live_typing::is_erase_safe(
                                                        delta,
                                                    ) {
                                                        typer.feed(delta.to_string());
                                                        typed_committed = text.committed.clone();
                                                    } else {
                                                        // Non-ASCII scalars break the one-scalar-
                                                        // one-backspace erase invariant — freeze.
                                                        debug!("Live typing: non-ASCII committed text; freezing");
                                                        live_feeding = false;
                                                    }
                                                }
                                                LiveDelta::Rewrite => {
                                                    // Committed text is documented append-only;
                                                    // if it ever rewrites, freeze live output and
                                                    // let the finalize erase reconcile the screen.
                                                    debug!("Live typing: committed prefix rewrote; freezing");
                                                    live_feeding = false;
                                                }
                                            }
                                        }
        ...
        self.return_engine(engine, &model_id);
        // Erase the live-typed prefix BEFORE releasing finalize (and on the
        // cancel path), so the final paste replaces it instead of appending.
        if let Some(typer) = live_typer.take() {
            typer.erase_all_and_wait(Duration::from_secs(10));
        }
...
fn post_process_transcription_text(
    ...
    if settings.spoken_punctuation_enabled {
        crate::audio_toolkit::apply_spoken_punctuation(&filtered)
    } else {
        filtered
    }
}
