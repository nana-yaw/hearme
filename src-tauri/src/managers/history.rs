...
impl HistoryManager {
    ...
    /// Most recent transcripts, newest first — input for dictionary mining.
    pub async fn recent_transcripts(&self, limit: usize) -> Result<Vec<String>> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
            "SELECT transcription_text FROM transcription_history ORDER BY id DESC LIMIT ?1",
        )?;
        let texts = stmt
    ...
    /// Case-insensitive substring search over transcript, post-processed
    /// text, and title. User input is LIKE-escaped so wildcards match
    /// literally; the query itself is parameterized.
    pub async fn search_history_entries(
        ...
        let entries = stmt
            .query_map(params![pattern, limit], Self::map_history_entry)?
            .collect::<std::result::Result<Vec<_>, _>>()?;
        Ok(PaginatedHistory {
            entries,
            has_more: false,
