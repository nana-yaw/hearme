...
fn initialize_core_logic(app_handle: &AppHandle) {
    ...
    app_handle.manage(Arc::new(commands::benchmark::BenchmarkState::default()));

    // Fork: pre-load the selected model so the first dictation after launch is
    // instant and the model indicator starts green. "Immediately" is the
    // explicit on-demand policy — respect it and skip. Off the main thread so
    // startup stays snappy; the loading guard bows out if something else is
    // already loading.
    {
        let preload_tm = transcription_manager.clone();
        let preload_ah = app_handle.clone();
        std::thread::spawn(move || {
            let settings = crate::settings::get_settings(&preload_ah);
            ...
            let Some(_loading_guard) = preload_tm.try_start_loading() else {
                return;
            };
            if let Err(e) = preload_tm.load_model(&settings.selected_model) {
                log::warn!(
                    "Startup model pre-load failed for '{}': {}",
...
fn run_headless_transcription(app: &AppHandle, args: &CliArgs) -> i32 {
    ...
    // Benchmarkable mirror of the GUI noise_suppression_enabled setting.
...
pub fn run(cli_args: CliArgs) {
    ...
    let specta_builder = Builder::<tauri::Wry>::new()
        .commands(collect_commands![
            ...
            shortcut::change_live_typing_setting,
            shortcut::change_accent_color_setting,
            shortcut::get_frontmost_app_bundle_id,
            shortcut::change_app_profiles_setting,
            ...
            commands::transcription::load_model_manually,
            commands::benchmark::benchmark_start_recording,
            commands::benchmark::benchmark_stop_recording,
            commands::benchmark::benchmark_cancel_recording,
            commands::benchmark::benchmark_discard_samples,
            commands::benchmark::run_model_benchmark,
            commands::benchmark::cancel_model_benchmark,
            commands::history::get_history_entries,
            commands::history::search_history_entries,
            commands::history::get_dictionary_suggestions,
