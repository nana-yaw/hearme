...
public func processTextWithSystemPrompt(
    ...
    Task.detached(priority: .userInitiated) {
        ...
        do {
            ...
            // plugin, which ships only with full Xcode — this fork builds with Command
            // Line Tools, so we use upstream's runtime-fallback plain-text path directly.
            let generation = try await session.respond(to: swiftUserContent)
            var output: String = generation.content
